...
public class GameListener implements Listener {
    ...
    private final String tsn = ChatColor.GOLD + "追踪棒 " + ChatColor.GREEN + "剩余次数: ";
    private final String lwn = "使用次数: ";
        ...
        if (damager instanceof Player) {
            ...
            if (player.getInventory().getItemInMainHand().getType().equals(Material.DIAMOND_SWORD)) {
                ...
                if (im.hasDisplayName()) {
                    if (Objects.requireNonNull(im.getDisplayName()).contains(lwn)) {
                        int uses = Integer.parseInt(Objects.requireNonNull(im.getDisplayName()).replace(lwn, ""));
                        if (uses == 0) {
                            event.setCancelled(true);
                            Util.scm(player, lang.track_empty);
                        } else {
                            im.setDisplayName(lwn + (uses - 1));
                            Util.scm(player, lang.track_no_near);
                        }
                    }
                }
            }
